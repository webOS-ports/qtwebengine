...
content::WebContents *WebContentsDelegateQt::OpenURLFromTab(content::WebContents *source, const content::OpenURLParams &params)
{
    ...
    if (params.disposition != WindowOpenDisposition::CURRENT_TAB) {
        ...
        QSharedPointer<WebContentsAdapter> targetAdapter = createWindow(nullptr, params.disposition, gfx::Rect(), toQt(params.url), params.user_gesture, additional_features);
        if (targetAdapter) {
...
void WebContentsDelegateQt::AddNewContents(content::WebContents* source, std::unique_ptr<content::WebContents> new_contents, const GURL &target_url,
                                           WindowOpenDisposition disposition, const gfx::Rect& initial_pos, bool user_gesture, bool* was_blocked, std::vector<base::string16> additional_features)
{
    Q_UNUSED(source)
    QSharedPointer<WebContentsAdapter> newAdapter = createWindow(std::move(new_contents), disposition, initial_pos, toQt(target_url), user_gesture, additional_features);
...
WebContentsDelegateQt::createWindow(std::unique_ptr<content::WebContents> new_contents,
                                    ...
                                    bool user_gesture, std::vector<base::string16> additional_features)
{
    ...
    return m_viewClient->adoptNewWindow(
            ...
            toQt(initial_pos), url, additionalFeaturesStringList);
}
